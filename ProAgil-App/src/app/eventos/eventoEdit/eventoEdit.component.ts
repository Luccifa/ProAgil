...
carregarEvento() {
  ...
  this.eventoService.getEventoById(idEvento)
    .subscribe(
      (evento: Evento[]) => {
        this.evento[0] = Object.assign({}, evento);
        console.log(this.evento[0]);
        this.fileNameToUpdate = this.evento[0].imagemURL.toString();

        this.imagemURL = `http://localhost:5000/Resources/images/${this.evento[0].imagemURL}?_ts=${this.dataAtual}`;

        this.evento[0].imagemURL = '';
        this.registerForm.patchValue(this.evento[0]);

        this.evento[0].lotes.forEach(lote => {
          this.lotes.push(this.criaLote(lote));
        });

        this.evento[0].redesSociais.forEach(redeSocial => {
